...
namespace LDA
{
	class HMM
	{
		...
		static public HMM getInstance(List<int[]> On, int wordnum, int topicNum, double alpha, double beta, int seed)
		{
			return new HMM(On, wordnum, topicNum, alpha, beta, seed);
		}

		public HMM(List<int[]> On, int wordnum, int topicNum, double alpha, double beta, int seed)
		{
			data = TokenData.loadData(On, wordnum);
		...
		public void sampling(int iteration)
		{
			for (int it = 0; it < iteration; it++)
			{
				...
				Console.WriteLine("\t\ttime(sec):" + time + "\ttime(word/sec):" + wps + "\tperplexity:" + perplexity());
			}
		}
		...
		private int selectTopic(int n, int i)
		{
			...
			for (int z = 0; z < topicNum; z++)
			{
				if (i == 0)
				{
					prob[z] = ((double)countZ0[assign[n][i]] + alpha) * ((double)countZ_Z[z, assign[n][i + 1]] + alpha) * ((double)countW_Z[z, data.tokens(n)[i]] + beta);
				}
